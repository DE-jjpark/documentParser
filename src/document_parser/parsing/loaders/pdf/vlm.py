...
표(table) 처리: pymupdf 텍스트 추출(native)은 표의 글자는 뽑아도 행/열/병합
구조를 몰라서 TEDS/TEDS-S 평가에서 점수가 나올 수가 없다 — "표는 plumber가
아니라 VLM/DI로 보낸다"는 설계 결정에 따라 table 라벨도 layout.py의
_FIGURE_LABELS에 포함시켜 여기로 오게 했고(text_boxes에서는 빠짐), VLM한테
캡션이 아니라 HTML 표 마크업 자체를 요청한다.
"""
...
_TABLE_PROMPT = (
    "Extract the table shown in this image as clean HTML markup. Output ONLY a "
    "single <table>...</table> element with <tr>/<td> rows and cells (use "
    "colspan/rowspan attributes for merged cells if present). Do not include any "
    "explanation, markdown code fences, or text outside the <table> tag."
)
...
def _strip_html_tags(html: str) -> str:
    """표 HTML에서 plain text 버전을 뽑아낸다 — DocumentElement.text는
    포맷 무관 일반 텍스트 필드라(청킹 등에서 그대로 씀) 태그를 남겨두면 안 됨.
    실제 구조(행/열/병합)는 metadata["html"]에 그대로 남아있다."""
